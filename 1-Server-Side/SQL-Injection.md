# PHẦN 1: GIỚI THIỆU VỀ SQL INJECTION (SQLi)
...
#### 1. Cấu trúc bảng mục tiêu (Giả định)
Hãy tưởng tượng bên trong Database, bảng `products` đang có cấu trúc và dữ liệu như sau:

| id  | name          | price |
| --- | ------------- | ----- |
| 101 | Laptop Gaming | 2500  |
| 102 | Laptop Lenovo | 2200  |

---

#### 2. Thử nghiệm thực tế (Trial & Error)

Chúng ta sẽ thực hiện các cuộc "tiêm kích" để xem Database phản ứng thế nào khi ta cố tình chèn một chuỗi ký tự `'abc'` vào các vị trí khác nhau trong tập kết quả trả về.

**Tình huống A: Nối thêm một hàng có cột kiểu chuỗi vào cột Số (Lỗi)**
Bạn gửi Payload:
```sql
' UNION SELECT 'abc', NULL, NULL--
```
**Cấu trúc dữ liệu mà Database nhận được lúc này:**
<table>
  <tr>
    <th bgcolor="#f8d7da">Cột 1 (id)</th>
    <th>Cột 2 (name)</th>
    <th>Cột 3 (price)</th>
  </tr>
  <tr>
    <td><code>102</code></td>
    <td><code>'Laptop Lenovo'</code></td>
    <td><code>2500</code></td>
  </tr>
  <tr>
    <td><code>101</code></td>
    <td><code>'Laptop Gaming'</code></td>
    <td><code>2500</code></td>
  </tr>
  <tr>
    <td style="color: red; font-weight: bold;">'abc' <br> (Sai kiểu dữ liệu!)</td>
    <td><code>NULL</code></td>
    <td><code>NULL</code></td>
  </tr>
</table>

> ❌ **Kết quả:** Database báo lỗi ngay lập tức vì không thể nhét chữ `'abc'` vào cái lỗ dành cho số `INT`. Trang web hiện lỗi **500 Internal Server Error**.

---

**Tình huống B:  Nối thêm một hàng có cột kiểu chuỗi vào cột chuỗi (Thành công)**
Bạn gửi Payload:
```sql
...
<table>
  <tr>
    <th>Cột 1 (id)</th>
    <th bgcolor="#d4edda">Cột 2 (name)</th>
    <th>Cột 3 (price)</th>
  </tr>
  <tr>
    <td><code>101</code></td>
  ...
  <tr>
    <td><code>NULL</code></td>
    <td style="color: green; font-weight: bold;">'abc' <br> (Khớp kiểu dữ liệu!)</td>
    <td><code>NULL</code></td>
  </tr>
</table>
...
<details>
<summary><b>💡 Tại sao bước này là bắt buộc? (Click để xem)</b></summary>

---
    
*   **Tính nghiêm ngặt của SQL:** Database giống như một người thủ kho khó tính, bạn không thể để một chiếc xe đạp (Chuỗi) vào vị trí dành cho một con số (Số).
*   **Điểm hiển thị:** Chỉ khi bạn tìm thấy cột kiểu chuỗi và nó **hiện ra trên giao diện**, bạn mới có thể đọc được Password hay Username mà bạn đánh cắp.
*   **Giải pháp dự phòng:** Nếu bạn lo lắng bài Lab "toàn cột số", hãy nhớ rằng PortSwigger luôn để sẵn ít nhất một cột chuỗi cho bạn. Trong thực tế, nếu gặp bảng toàn số, chúng ta sẽ chuyển sang kỹ thuật **Blind SQLi** (lấy dữ liệu qua các câu hỏi Đúng/Sai thay vì hiển thị trực tiếp).
